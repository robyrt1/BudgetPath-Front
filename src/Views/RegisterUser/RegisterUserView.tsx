...
const RegisterUserView = () => {
    const { name, nameError, setName, email, emailError, setEmail, password, passwordError, errorResponse, setPassword, register } = UseRegisterUserViewModel();
    return (
        ...
        <div className='bg-white dark:bg-gray-800 rounded-lg px-6 py-8 ring shadow-xl ring-gray-900/5 flex flex-col items-center justify-center min-h-[300px]'> {/* Ajuste a classe h-screen para min-h-screen */}
            ...
            {errorResponse && <span className='text-red-600 text-sm'>{errorResponse}</span>}



            <button disabled={!!emailError || !!passwordError || !email || !password || !name} className="mt-4 p-2 bg-blue-500 text-white rounded-md" onClick={register}>
                Cadastrar
            </button>
        </div>
    );
